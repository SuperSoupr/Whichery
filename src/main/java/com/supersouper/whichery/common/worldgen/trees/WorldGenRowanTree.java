...
public class WorldGenRowanTree extends WorldGenAbstractTree {
    ...
    public WorldGenRowanTree(boolean notify) {
        super(notify);
    }
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int ty = y; ty <= top; ty++) {
            setBlockAndNotifyAdequately(world, x, ty, z, ModBlocks.ROWAN_LOG.get(), 0);
        }
            ...
            for (int lx = x - r; lx <= x + r; lx++) {
                for (int lz = z - r; lz <= z + r; lz++) {
                    ...
                    if (existing.isAir(world, lx, ly, lz) || existing instanceof BlockLeaves) {
                        setBlockAndNotifyAdequately(world, lx, ly, lz, ModBlocks.ROWAN_LEAVES.get(), 0);
                    }
                }
            }
        }
